...
class ACajaModel
{
    ...
    public function Actualizar(Caja $data)
    {
     ...
     /**
     * Consulta los datos pertenecientes a una caja
     * @param $id
     */
    public function DatosCaja($data)
    {
        try
        {
            $stm = $this->conexionn->prepare("SELECT * FROM v_caja_aper WHERE estado <> 'c' AND id_apc=".$data['cod_apc']."");
            $stm->execute();
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            $fecha_a = date('Y-m-d H:i:s',strtotime($data['fecha_a']));
            $fecha_c = date('Y-m-d H:i:s',strtotime($data['fecha_c']));
         
           
            $queryIngresos = $this->conexionn->prepare("SELECT IFNULL(SUM(importe),0) AS total_i FROM tm_ingresos_adm WHERE
            (fecha_reg >='".$data['fecha_a']."' AND fecha_reg <= '".$data['fecha_c']."')
             AND id_apc =5 AND estado='a'");
            $queryIngresos->execute();
            $ingresos = $queryIngresos->fetch(PDO::FETCH_ASSOC);

            $queryGastos = $this->conexionn->prepare("SELECT IFNULL(SUM(importe),0) AS total_g FROM v_gastosadm WHERE
            (fecha_re >='".$data['fecha_a']."' AND fecha_re <= '".$data['fecha_c']."')
             AND id_apc =5 AND estado='a'");
            $queryGastos->execute();
            $gastos = $queryGastos->fetch(PDO::FETCH_ASSOC);
           
            $response['data'] = $c;
            $response['data']['ingresos'] = $ingresos; 
            $response['data']['gastos'] = $gastos;  
            //$data = array("data" => $c);
